"""
FLUX.2-klein - MLX Native Single Image Editing on Mac

Uses mflux (MLX port) for Apple Silicon acceleration.
Edit a single image with text prompt using img2img with strength control.
...
def load_model(model="flux2-klein-9b", quantize: int = 8):
    ...
    print(f"  Mode: Single-image img2img editing")
...
def edit_image(
    ...
    """Edit a single image using text prompt (image-to-image).
    ...
    Args:
        ...
        image_strength: How strongly the input image influences output (0.0-1.0).
                      ...
                      Use 0.3-0.5 to preserve more of original (fewer denoising steps).
        seed: Random seed
    ...
    # Calculate init timestep if image_strength is set
...
def main():
    parser = argparse.ArgumentParser(
        description="Edit a single image with FLUX.2-klein using MLX (mflux). "
                    "Uses img2img with strength control to preserve input image."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Image strength: controls how many denoising steps to skip (0.0-1.0). "
             "Default None means full denoising (same as CLI). "
             "Use 0.3-0.5 to preserve more of original image (skips some denoising steps). "
             "(default: None)",
    )
